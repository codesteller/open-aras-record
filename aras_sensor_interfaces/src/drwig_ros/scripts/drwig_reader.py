...
class Drwig1843Reader(DrwigReaderBase):
    ...
    def parseTrackedObjects(self, bytevecStr, byteVecIndex, tlvLength, trackedObjects):
        ...
        if tlvLength > 0:
            ...
            elevation = np.zeros(trackedObjects["num_objects"])
...
class DrwigReader:
    ...
    def run(self):
        ...
        while True:
            ...
            # Estimate the time taken to read the data
            time_tic = time.time()
            ...
            if parsed_data is None:
                ...
                # time.sleep(0.1)
                if parsed_data_last_frame is not None:
                    # continue
                    parsed_data = parsed_data_last_frame
                else:
                    continue

            # Estimate the time taken to read the data'
            if self.dw.debug:
                rospy.loginfo("Time taken to read the data: %f" % (time.time() - time_tic))

            # --------------------------------------------------------------------------
            #               Detection Data
            # --------------------------------------------------------------------------
            detectedObjects = parsed_data["detected_objects"]

            if detectedObjects["num_objects"] < 1:
                # if self.dw.debug:
            ...
            if _flag:
                ...
                for i in range(num_points):
                    ...
                    points[i, 2] = trackedObjects["elevation"][i]
